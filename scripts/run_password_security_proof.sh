...
cache_root="${XDG_CACHE_HOME:-${HOME}/.cache}/despesas-frontend-password-proof"
...
attempt_timeout_seconds="${PROOF_ATTEMPT_TIMEOUT_SECONDS:-180}"
define_file="$(mktemp)"
chromedriver_pid=""
...
ensure_chromedriver() {
  local chrome_version driver_root zip_path extracted_dir driver_bin
  ...
  "${driver_bin}" --port=4444 >/tmp/despesas-frontend-password-proof-chromedriver.log 2>&1 &
  chromedriver_pid="$!"
  ...
  echo "Unable to start chromedriver" >&2
  exit 1
}
...
cat > "${define_file}" <<EOF
{
  "APP_ENV": "password-proof",
  "API_BASE_URL": "${API_BASE_URL}",
  "PROOF_RUN_ID": "${proof_run_id}",
  "PROOF_ADMIN_EMAIL": "${PROOF_ADMIN_EMAIL}",
  "PROOF_ADMIN_PASSWORD": "${PROOF_ADMIN_PASSWORD}"
}
EOF
...
while (( attempt <= max_attempts )); do
  ...
  timeout --signal=TERM "${attempt_timeout_seconds}" flutter drive \
    --driver=test_driver/password_security_proof.dart \
    --target=integration_test/password_security_proof_test.dart \
    -d chrome \
    --browser-dimension=1440x1200@1 \
    "${browser_args[@]}" \
